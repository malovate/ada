...
// We use both: AdaFace SVG for animations, and the real photo
// behind it at low opacity for atmosphere.
...
export default function VoiceScreen({ adaMood, onBack, onNewMessage }) {
  ...
  return (
    <div style={styles.screen}>
      ...
      {/* ── Ada's Face — photo + animated SVG overlay ── */}
      <div style={styles.faceArea}>
        {/* Her real photo — full background of the face area */}
        <img
          src="/ada-avatar.png"
          alt="Ada"
          style={{
            ...styles.facePhoto,
            // Dim more when speaking (focus on animation)
            // Dim less when idle (feel her presence)
            opacity: faceState === 'speaking' ? 0.55 :
                     faceState === 'thinking' ? 0.45 : 0.7,
            transition: 'opacity 0.8s ease',
          }}
        />
        {/* Gradient overlay so the photo blends into the dark background */}
        <div style={styles.facePhotoGradient} />
        {/* Animated SVG face on top — handles state animations */}
...
const styles = {
  ...
  facePhoto: {
    ...
    objectFit: 'cover',
    objectPosition: 'top',
    // objectPosition: top keeps her face visible (not the background)
    zIndex: 1,
  },
  ...
  facePhotoGradient: {
    ...
    width: '100%',
    height: '100%',
    // Gradient: transparent at top (show face), dark at bottom (blend into mic area)
    background: 'linear-gradient(to bottom, rgba(10,10,15,0.1) 0%, rgba(10,10,15,0.7) 85%, rgba(10,10,15,1) 100%)',
    zIndex: 2,
  },

  faceOverlay: {
    position: 'relative',
    zIndex: 3,   // SVG animations on top of the photo
    width: '100%',
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
