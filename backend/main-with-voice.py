...
# speak            = Ada's text → MP3 audio (ElevenLabs)
...
class VoiceMessageResponse(BaseModel):
    ...
    Audio is fetched separately via /voice/speak.
    """
...
async def ada_background_thinking():
    """Runs every 2 hours. Ada thinks even when Paul is offline."""
...
async def voice_message_endpoint(audio: UploadFile = File(...)):
    ...
    # ── Step 1: Speech → Text ─────────────────────────────
    # Send audio bytes to ElevenLabs Scribe for transcription.
    # 'audio.filename' = original filename, e.g. "recording.m4a"
...
async def list_voices():
    """
    Lists available ElevenLabs voices via their API.
    Use this to confirm Rachel is available and find her exact ID.

    Visit: http://localhost:5000/voice/voices
    Look for "Rachel" in the response — her ID is 21m00Tcm4TlvDq8ikWAM
    """
